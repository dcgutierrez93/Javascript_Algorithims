...
// (2) At each element check if it exists in the hash table, which has a lookup
//     of O(1) time
// (3) If the element exists in the hash table then it is a duplicate, if it
//     doesn't exist, insert it into the hash table, also O(1)
...
function duplicates(arr) {
    ...
    for (var i = 0; i < arr.length; i++) {
        ...
        if (hashTable[arr[i].toString()] === undefined) {
            hashTable[arr[i].toString()] = true// if element does exist in hash table
